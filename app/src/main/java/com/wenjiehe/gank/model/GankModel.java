...
public class GankModel implements IModel {
    ...
    private static volatile GankModel sInstance;

    public static GankModel getInstance(Context context) {
        if (sInstance == null) {
            synchronized (GankModel.class) {
                if (sInstance == null) {
                    sInstance = new GankModel(context);
                }
            }
        }
    ...
    private GankModel(Context context) {
    ...
    private Observable<List<GankItem>> loadFromDay(final String day) {
        ...
        if (isOver(day)) {
            ...
            return load(day).flatMap(new Func1<List<GankItem>, Observable<List<GankItem>>>() {
                @Override
